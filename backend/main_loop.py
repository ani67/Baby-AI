...
class MainLoop:
    ...
    def cycle(
        ...
        *,
        ...
        # strongest active concept. The decision_record retains D's
        # original choice — only the outward act is overridden.
        if force_respond and action.action is ActionKind.WAIT and spread.active_set:
            top_cid = max(spread.active_set, key=lambda c: spread.active_set[c])
            action = ActionDescriptor(
        ...
        if action.action is ActionKind.EXPRESS:
            # internal_repr = weighted centroid of the active set. This is what
            # the mind currently HAS IN MIND, not just the bare input. With
            # auto-linked similar_to edges + spread, this centroid reflects
            # both the input and the related thoughts that surfaced — so the
            # candidate G picks isn't a strict echo of what the user said
            # but the closest surface to the broader mental state.
            internal_repr = self._active_set_centroid(spread.active_set)
            ...
            intent = ExpressionIntent(
                ...
                active_concepts=dict(spread.active_set),
                now=now,
